...
W_sensors = (rng.rand(rank, train_data.shape[1]) - 0.5).astype('float32') / 10.
W_time = (rng.rand(train_data.shape[2], rank) - 0.5).astype('float32') / 10.
# the matrix from the low rank representation to joint output and offset
W_joint_outputs = (rng.rand(rank, n_joint_outputs) - 0.5
                   ).astype('float32') / 10.
...
SW_time = theano.shared(W_time.T, name='SW_time')
...
time_projections = (
    sensor_projections * SW_time.dimshuffle('x', 0, 1)).sum(-1)

joint_outputs_raw = time_projections.dot(SW_joint_outputs)
...
smoothness_penalty = ((SW_time[:, 1:] - SW_time[:, :-1]) ** 2).mean()
penalty_factor = 1000.
...
learning_rate = .01
batch_size = 50
updates = [(SW_sensors, SW_sensors - learning_rate * grad_log_lik[0]),
           (SW_time, SW_time - learning_rate * grad_log_lik[1]),
           (SW_joint_outputs, SW_joint_outputs -
            learning_rate * grad_log_lik[2]),
...
W_l2_targets = rng.rand(n_joint_outputs, 2).astype('float32') / 10.
b_l2_targets = np.zeros(2, dtype=np.float32) / 10.
...
if __name__ == "__main__":
    ...
    f_everything_classify = theano.function([input_data], l2_classify)
    ...
    from utils import load_test_subjects
    X_test, labels_test = load_test_subjects()

    predictions = f_everything_classify(X_test)

    indices = np.zeros_like(predictions)
    for l in np.unique(labels_test):
        indices[labels_test == l] = 1000 * l + np.arange(
            (labels_test == l).sum())

    submission_file = "submission_nnet.csv"
